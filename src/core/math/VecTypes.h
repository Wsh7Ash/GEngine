...
template <typename T = float> struct Vec2 {
  static_assert(std::is_arithmetic_v<T>,
                "Vec2<T> requires an arithmetic scalar.");
  ...
  [[nodiscard]] Vec2
  NormalizedSafe(const Vec2 &fallback = Vec2::UnitX()) const noexcept {
    const T len = Length();
  ...
  [[nodiscard]] constexpr Vec2 Clamp(const Vec2 &lo,
                                     const Vec2 &hi) const noexcept {
    return {Math::Clamp(x, lo.x, hi.x), Math::Clamp(y, lo.y, hi.y)};
  }

  /// Approximate equality using Math::ApproxEqual per component.
  [[nodiscard]] constexpr bool
  ApproxEqual(const Vec2 &rhs, T eps = Constants<T>::EPSILON) const noexcept {
...
// Scalar * Vec2
template <typename T>
[[nodiscard]] constexpr Vec2<T> operator*(T s, const Vec2<T> &v) noexcept {
  return v * s;
}
...
template <typename T>
[[nodiscard]] constexpr T DistanceSq(const Vec2<T> &a,
...
template <typename T>
[[nodiscard]] constexpr Vec2<T> Lerp(const Vec2<T> &a, const Vec2<T> &b,
                                     T t) noexcept {
  return a.Lerp(b, t);
}
...
//  Vec3<T>
// ================================================================
template <typename T = float> struct alignas(16) Vec3 {
  static_assert(std::is_arithmetic_v<T>,
                "Vec3<T> requires an arithmetic scalar.");
...
public:
  ...
  explicit constexpr Vec3(T scalar) noexcept
      : x(scalar), y(scalar), z(scalar) {}

  /// Construct from Vec2 + optional z.
  constexpr Vec3(const Vec2<T> &v, T z_ = T{0}) noexcept
      : x(v.x), y(v.y), z(z_) {}
  ...
  [[nodiscard]] Vec3
  NormalizedSafe(const Vec3 &fallback = Vec3::UnitX()) const noexcept {
    const T len = Length();
    return (len > Constants<T>::EPSILON) ? (*this / len) : fallback;
  }
  ...
  [[nodiscard]] constexpr T DistanceSq(const Vec3 &other) const noexcept {
    return (*this - other).LengthSq();
  }
  ...
  /// Refract through a surface (Snell's law). `eta` = ratio of indices of
  /// refraction. Returns zero vector on total internal reflection.
  ...
  [[nodiscard]] constexpr Vec3 Clamp(const Vec3 &lo,
                                     const Vec3 &hi) const noexcept {
    return {Math::Clamp(x, lo.x, hi.x), Math::Clamp(y, lo.y, hi.y),
            Math::Clamp(z, lo.z, hi.z)};
  }

  [[nodiscard]] constexpr bool
  ApproxEqual(const Vec3 &rhs, T eps = Constants<T>::EPSILON) const noexcept {
    return Math::ApproxEqual(x, rhs.x, eps) &&
           Math::ApproxEqual(y, rhs.y, eps) && Math::ApproxEqual(z, rhs.z, eps);
  }
...
template <typename T>
[[nodiscard]] constexpr Vec3<T> operator*(T s, const Vec3<T> &v) noexcept {
  return v * s;
}
...
template <typename T>
[[nodiscard]] constexpr Vec3<T> Cross(const Vec3<T> &a,
...
template <typename T>
[[nodiscard]] constexpr T DistanceSq(const Vec3<T> &a,
                                     const Vec3<T> &b) noexcept {
  return a.DistanceSq(b);
}
template <typename T>
...
template <typename T>
[[nodiscard]] constexpr Vec3<T> Lerp(const Vec3<T> &a, const Vec3<T> &b,
                                     T t) noexcept {
  return a.Lerp(b, t);
}
template <typename T>
[[nodiscard]] constexpr Vec3<T> Reflect(const Vec3<T> &v,
                                        const Vec3<T> &n) noexcept {
  return v.Reflect(n);
}
...
//  Vec4<T>
// ================================================================
template <typename T = float> struct alignas(16) Vec4 {
  static_assert(std::is_arithmetic_v<T>,
                "Vec4<T> requires an arithmetic scalar.");
  ...
  [[nodiscard]] static constexpr Vec4 UnitX() noexcept {
    return {T{1}, T{0}, T{0}, T{0}};
  }
  [[nodiscard]] static constexpr Vec4 UnitY() noexcept {
    return {T{0}, T{1}, T{0}, T{0}};
  }
  [[nodiscard]] static constexpr Vec4 UnitZ() noexcept {
    return {T{0}, T{0}, T{1}, T{0}};
  }
  [[nodiscard]] static constexpr Vec4 UnitW() noexcept {
    return {T{0}, T{0}, T{0}, T{1}};
  }

  // Colour helpers (interprets xyzw as rgba)
  [[nodiscard]] static constexpr Vec4 Black() noexcept {
    return {T{0}, T{0}, T{0}, T{1}};
  }
  [[nodiscard]] static constexpr Vec4 White() noexcept {
    return {T{1}, T{1}, T{1}, T{1}};
  }
  [[nodiscard]] static constexpr Vec4 Red() noexcept {
    return {T{1}, T{0}, T{0}, T{1}};
  }
  [[nodiscard]] static constexpr Vec4 Green() noexcept {
    return {T{0}, T{1}, T{0}, T{1}};
  }
  [[nodiscard]] static constexpr Vec4 Blue() noexcept {
    return {T{0}, T{0}, T{1}, T{1}};
  }
  [[nodiscard]] static constexpr Vec4 Transparent() noexcept {
    return {T{0}, T{0}, T{0}, T{0}};
  }
  ...
  constexpr Vec4(T x_, T y_, T z_, T w_) noexcept
      : x(x_), y(y_), z(z_), w(w_) {}
  explicit constexpr Vec4(T scalar) noexcept
      : x(scalar), y(scalar), z(scalar), w(scalar) {}

  /// Construct from Vec3 + w  (common for homogeneous coordinates).
  constexpr Vec4(const Vec3<T> &v, T w_ = T{0}) noexcept
  ...
  [[nodiscard]] constexpr T &a() noexcept { return w; }
  ...
  // Compound assignment — scalar
  // ----------------------------------------------------------
  constexpr Vec4 &operator*=(T s) noexcept {
    x *= s;
    y *= s;
    z *= s;
    w *= s;
  ...
  [[nodiscard]] Vec4
  NormalizedSafe(const Vec4 &fallback = Vec4::UnitX()) const noexcept {
    const T len = Length();
    return (len > Constants<T>::EPSILON) ? (*this / len) : fallback;
  }
  ...
  [[nodiscard]] T Distance(const Vec4 &other) const noexcept {
    return (*this - other).Length();
  }
  [[nodiscard]] constexpr T DistanceSq(const Vec4 &other) const noexcept {
    return (*this - other).LengthSq();
  }
  ...
  [[nodiscard]] constexpr Vec4 Clamp(const Vec4 &lo,
                                     const Vec4 &hi) const noexcept {
    return {Math::Clamp(x, lo.x, hi.x), Math::Clamp(y, lo.y, hi.y),
            Math::Clamp(z, lo.z, hi.z), Math::Clamp(w, lo.w, hi.w)};
  }

  [[nodiscard]] constexpr bool
  ApproxEqual(const Vec4 &rhs, T eps = Constants<T>::EPSILON) const noexcept {
    return Math::ApproxEqual(x, rhs.x, eps) &&
           Math::ApproxEqual(y, rhs.y, eps) &&
           Math::ApproxEqual(z, rhs.z, eps) && Math::ApproxEqual(w, rhs.w, eps);
  }
...
template <typename T>
[[nodiscard]] constexpr Vec4<T> operator*(T s, const Vec4<T> &v) noexcept {
  return v * s;
}
...
template <typename T>
[[nodiscard]] constexpr Vec4<T> Cross3(const Vec4<T> &a,
...
template <typename T>
[[nodiscard]] constexpr T DistanceSq(const Vec4<T> &a,
                                     const Vec4<T> &b) noexcept {
  return a.DistanceSq(b);
}
template <typename T>
[[nodiscard]] inline Vec4<T> Normalize(const Vec4<T> &v) noexcept {
  return v.Normalized();
}
template <typename T>
[[nodiscard]] constexpr Vec4<T> Lerp(const Vec4<T> &a, const Vec4<T> &b,
                                     T t) noexcept {
  return a.Lerp(b, t);
}
