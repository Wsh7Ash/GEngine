...
void Renderer2D::Init() {
  ...
  s_Data.TextureShader = Shader::Create(shaderRoot + "sprite_batch.vert",
                                        shaderRoot + "sprite_batch.frag");
  uint32_t whiteTextureData = 0xffffffff;
  s_Data.WhiteTexture =
      Texture::Create(1, 1, &whiteTextureData, sizeof(uint32_t));
  ...
  // For now, let's assume CreateDynamic can take initial indices or we add
  // SetIndices Actually, CreateDynamic just allocates. We'll use a specialized
  // constructor or just SetData for indices too if needed. But indices are
  // static, so we can just create a Mesh with them.
...
void Renderer2D::DrawQuad(const Math::Vec2f &position, const Math::Vec2f &size,
                          const Math::Vec4f &color, int entityID) {
...
void Renderer2D::DrawQuad(const Math::Vec3f &position, const Math::Vec2f &size,
  ...
  for (int i = 0; i < 4; i++) {
    s_Data.QuadVertexBufferPtr->Position[0] =
        position.x + (s_Data.QuadVertexPositions[i].x * size.x);
    s_Data.QuadVertexBufferPtr->Position[1] =
